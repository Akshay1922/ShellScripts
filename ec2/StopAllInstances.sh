# Stop All instances which are in stopped state
REGION_NAME=$1
if [ $REGION_NAME = ""]; then
...
while [ $a -lt $Instance_Count ]
    do
        ...
        if [ $Instance_state != "stopped" ]; then
            output=$(aws ec2 stop-instances --instance-ids $Instance_id --query 'StoppingInstances[*].CurrentState[].Name' --output text)
            echo "Now $Instance_id this instance going in $output state"
        else
            echo "$Instance_id this instance is Already stopped"
        fi
        a=`expr $a + 1`
    done